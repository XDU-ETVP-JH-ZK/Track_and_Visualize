...
//    QString path = QDir::currentPath();     //删除data文件夹下所有jpg文件和txt文件
//    QDir dir(path);
...
void MainWindow::on_s2run_clicked()     //输出选取点的物理坐标和像素坐标
{
    ...
    else if(!f.open(QIODevice::ReadOnly))
    {
        ...
        QString str = QString::number(x*d)+" "+QString::number(y*d)+" "+QString::number(pix_x)+" "+QString::number(pix_y);
        ui->s2txt->append(str);
        ...
        if(!ff.open(QIODevice::WriteOnly | QIODevice::Append))
            ...
            input<<QString::number(x*d)<<" "<<QString::number(y*d)<<" "<<QString::number(pix_x)<<" "<<QString::number(pix_y)<<"\n";
            ff.close();
            ui->in_x->clear();
            ui->in_y->clear();
            ui->in_x->setFocus();
...
void ImageItem::mousePressEvent(QGraphicsSceneMouseEvent* event)     //监听鼠标点击事件，点击后获取坐标并做标记
{
    ...
    if(n == 3){     //已选三次之后计算出均值，用作最终输入的像素坐标
        ...
        qDebug()<<x2<<" "<<y2;
        QString coordinate = QString::number(x2, 'f', 3) + " " + QString::number(y2, 'f', 3);     //精确到小数点后三位
        QDir dir;
        if(!dir.exists("data"))
            dir.mkdir("data");
...
//    pangolin::Var<string> pangolin_vt("menu.t", toutput);
    ...
    if (!video.read(frame)){
        qDebug() << "Could not read video file!!!";
        QMessageBox::critical(NULL, QString("出错了"),
                              QString("读取视频文件失败"),
                              QString("确定"));
    }
...
//                pangolin::FinishFrame();
                ...
                cout<<track.log<<endl;     //输出错误信息
                ui->s3log->append(QString::fromStdString(track.log));
            }
